...
function setupEditableElements() {
  ...
  allElements.forEach(element => {
    ...
    // Add hover effect with red dots
    element.addEventListener('mouseenter', () => {
      if (!isToolbarActive) {
        element.style.outline = '2px dashed #ff0000';
...
function activateElement(element) {
  ...
  element.style.outlineOffset = '2px';
  element.style.backgroundColor = 'rgba(255, 192, 0, 0.05)';
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/ai-chat', {
        ...
        body: JSON.stringify({ 
          message: currentEditableElement ? 
            `Help me edit this content: "${currentEditableElement.textContent.substring(0, 100)}..." User request: ${message}` : 
            message
        })
      });
      ...
      while (!done) {
        ...
        if (value) {
          ...
          if (cleanChunk && cleanChunk !== '[DONE]') {
            aiResponse.textContent += cleanChunk;
            messages.scrollTop = messages.scrollHeight;
          }
        }
      }
