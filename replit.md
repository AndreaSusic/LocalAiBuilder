...
- June 28, 2025: Enhanced chat interface with comprehensive business information collection - extended state schema to include social media (Facebook, Instagram, TikTok, LinkedIn), Google Business Profile, payment plans, and hero video fields, implemented intelligent auto-detection using regex patterns, increased chat input height from 42px to 80px for multi-line entry, added "Paste links one below other" instruction for social media, fixed social media field completion logic to properly advance to next questions
